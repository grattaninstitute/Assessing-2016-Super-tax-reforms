...
australia_adult_population_by_age_group_fy_year <- 
  ...
  mutate(fy.year = yr2fy(as.numeric(gsub(".Q[1-4]", "", obsTime)))) %>%
  group_by(working_age, fy.year) %>%
...
tax_expenditure_BAU_by_decile <- 
  ...
  group_by(`Taxable income decile`) %>%
...
present_prop_contr_ge_25k_by_decile <- function(decile){
  sample_file_1314 %>%
    ...
    filter(`Taxable income decile` %in% decile) %$%
    prop
}
@
...
modeled_sample_file_1718 %>%
  ...
  mutate(`Balance percentile` = ntile(MCS_Ttl_Acnt_Bal, 100)) %>%
  group_by(`Balance percentile`) %>%
...
prop_posttax_contr_by_taxpayers_whose_balance_exceeds <- function(threshold){
  sample_file_1314 %>%
    mutate(WEIGHT = 50) %>%
    apply_super_caps_and_div293() %>%
    mutate(exceeds_threshold = MCS_Ttl_Acnt_Bal > threshold) %>%
    group_by(exceeds_threshold) %>%
...
sample_file_1314 %>%
  mutate(WEIGHT = 50) %>%
  apply_super_caps_and_div293() %>% 
  mutate(`Super account balance` = cut(MCS_Ttl_Acnt_Bal, 
                                         ...
                                         ordered_result = TRUE, 
                                         include.lowest = TRUE)) %>%
  group_by(`Super account balance`) %>%
...
read_excel("data-raw/Chart-net-budget-impact.xlsx") %>%
  ...
  mutate(ypos = if_else(y > 0 & fill != "Net", y, NA_real_), 
         ...
         text = if_else(x == "2019-20" & fill != "Net", 
                        ...
                        NA_character_)) %>%
  mutate(.y_is_positive = y > 0, is_Net = fill == "Net") %>%
  group_by(x, .y_is_positive, is_Net) %>%
